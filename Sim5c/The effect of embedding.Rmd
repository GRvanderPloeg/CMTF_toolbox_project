...
inputLoadings = readRDS("./Sim5c_input_loadings_Y_inside.RDS")
...
```{r load ACMTF models}
ACMTFR_CV_models = readRDS("./Sim5c_ACMTFR_CV_Y_inside.RDS")
ACMTFR_CV_params = readRDS("./Sim5c_ACMTFR_CV_params_Y_inside.RDS")
...
Yreal = readRDS("./Sim5c_Y_Y_inside.RDS")
Yreal = Yreal@data
...
```
```{r check what the local components look like}
Y_found1 = rep(NA, length(ACMTFR_CV_models))
for(i in 1:length(ACMTFR_CV_models)){
  Y_found1[i] = max(abs(cor(ACMTFR_CV_models[[i]]$Fac[[1]], inputLoadingsFac[[1]][,3])))
}

Y_found2 = rep(NA, length(ACMTFR_CV_models))
for(i in 1:length(ACMTFR_CV_models)){
  Y_found2[i] = max(abs(cor(ACMTFR_CV_models[[i]]$Fac[[1]], inputLoadingsFac[[1]][,4])))
}

df = cbind(Y_found1, Y_found2, ACMTFR_CV_params)

df %>% as_tibble() %>% pivot_longer(-ACMTFR_CV_params) %>% ggplot(aes(x=as.factor(ACMTFR_CV_params),y=value)) + facet_wrap(~name) + geom_boxplot()

```

```{r another approach}
#' Compute component-wise recovery correlations between true and estimated loadings
#'
#' @param U      Numeric matrix of true loadings (I x r)
#' @param Uhat   Numeric matrix of estimated loadings (I x r)
#' @return       Named vector of length r with |correlations| for each component
component_recovery <- function(U, Uhat) {
  if (!is.matrix(U) || !is.matrix(Uhat)) {
    stop("Both U and Uhat must be matrices.")
  }
  if (ncol(U) != ncol(Uhat)) {
    stop("U and Uhat must have the same number of columns (components).")
  }
  r <- ncol(U)
  # 1) Compute Gram matrices
  G    <- crossprod(U)       # r x r
  Ghat <- crossprod(Uhat)    # r x r

  # 2) Eigen-decomposition for sqrt and inverse sqrt
  eigG    <- eigen(G)
  sqrtG   <- eigG$vectors %*% diag(sqrt(pmax(eigG$values, 0))) %*% t(eigG$vectors)
  invSqrtG <- eigG$vectors %*% diag(1 / sqrt(pmax(eigG$values, .Machine$double.eps))) %*% t(eigG$vectors)

  eigGhat    <- eigen(Ghat)
  sqrtGhat   <- eigGhat$vectors %*% diag(sqrt(pmax(eigGhat$values, 0))) %*% t(eigGhat$vectors)
  invSqrtGhat <- eigGhat$vectors %*% diag(1 / sqrt(pmax(eigGhat$values, .Machine$double.eps))) %*% t(eigGhat$vectors)

  # 3) Whitened (orthonormal) bases
  U_tilde    <- U     %*% invSqrtG
  Uhat_tilde <- Uhat  %*% invSqrtGhat

  # 4) Overlap and Procrustes alignment
  M  <- crossprod(U_tilde, Uhat_tilde)  # r x r
  sv <- svd(M)
  P  <- sv$u
  Q  <- sv$v
  R  <- Q %*% t(P)

  # 5) Rotate estimated back to original scale
  Uhat_aligned <- Uhat_tilde %*% R %*% sqrtG

  # 6) Compute per-component absolute correlations
  rho <- numeric(r)
  for (k in seq_len(r)) {
    rho[k] <- abs(cor(U[, k], Uhat_aligned[, k]))
  }
  names(rho) <- paste0("Comp", seq_len(r))
  return(rho)
}

component_recovery(inputLoadingsFac[[1]], ACMTFR_CV_models[[602]]$Fac[[1]])
...
result = do.call(rbind, lapply(ACMTFR_CV_models, FUN=function(x){component_recovery(x$Fac[[1]], inputLoadings[[1]])}))
colnames(result) = c("global1", "global2", "local1", "local2", "distinct1", "distinct2", "distinct3")

componentShapes = cbind(ACMTF_CV_FMS, result)
