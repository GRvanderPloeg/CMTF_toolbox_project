...
```{r run ACMTF}
model_ACMTF = acmtf_opt(Z, numComponents=5) # converges slowly due to only seeing 4 components
```
...
```{r run ACMTF}
model_ACMTF_YAS = acmtf_opt(Z_YAS, numComponents=5) # converges slowly due to only seeing 4 components
```
