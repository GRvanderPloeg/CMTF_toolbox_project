...
```{r run ACMTF}
model_ACMTF = acmtf_opt(Z, numComponents=5, abs_tol=1e-8, rel_tol=1e-8, max_iter=2) # converges slowly due to only seeing 4 components
```
...
```{r run ACMTF}
model_ACMTF_YAS = acmtf_opt(Z_YAS, numComponents=5, abs_tol=1e-8, rel_tol=1e-8, max_iter=2)
```
...
# ACMTF YAS
a = cbind(model_ACMTF_YAS$Fac[[1]][,1], a_global) %>% as_tibble() %>% ggplot(aes(x=a_global,y=V1)) + geom_point() + ggtitle("Common") + xlab("Real scores") + ylab("ACMTF-YAS")
b = cbind(model_ACMTF_YAS$Fac[[1]][,5], a_local) %>% as_tibble() %>% ggplot(aes(x=a_local,y=V1)) + geom_point() + ggtitle("Local") + xlab("Real scores") + ylab("ACMTF-YAS")
c = cbind(model_ACMTF_YAS$Fac[[1]][,2], a_distinct1) %>% as_tibble() %>% ggplot(aes(x=a_distinct1,y=V1)) + geom_point() + ggtitle("Distinct X1") + xlab("Real scores") + ylab("ACMTF-YAS")
d = cbind(model_ACMTF_YAS$Fac[[1]][,3], a_distinct2) %>% as_tibble() %>% ggplot(aes(x=a_distinct2,y=V1)) + geom_point() + ggtitle("Distinct X2") + xlab("Real scores") + ylab("ACMTF-YAS")
e = cbind(model_ACMTF_YAS$Fac[[1]][,4], a_distinct3) %>% as_tibble() %>% ggplot(aes(x=a_distinct3,y=V1)) + geom_point() + ggtitle("Distinct X3") + xlab("Real scores") + ylab("ACMTF-YAS")
