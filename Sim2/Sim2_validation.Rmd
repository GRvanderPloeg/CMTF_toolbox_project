...
correctModels = ACMTF_varExps %>% filter(X1 > threshold, X2 > threshold, X3 > threshold) %>% select(index) %>% pull()
...
ACMTF_CV_f = do.call(rbind, lapply(ACMTF_CV_models, FUN=function(x){x$f})) %>% as_tibble()
ACMTF_CV_f = ACMTF_CV_f %>% mutate(beta = ACMTF_CV_params, index=1:nrow(.))
ACMTF_CV_f %>% ggplot(aes(x=as.factor(beta),y=V1)) + geom_boxplot()
