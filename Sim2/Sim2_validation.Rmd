...
colnames(ACMTFR_FMS) = c("FMS", "beta", "pi")
...
ACMTFR_f %>% ggplot(aes(x=as.factor(beta),y=f)) + facet_wrap(~pi) + geom_boxplot()
ACMTFR_FMS %>% ggplot(aes(x=as.factor(beta),y=FMS)) + facet_wrap(~pi) + geom_boxplot()
```
...
componentShapes = cbind(ACMTFR_FMS, global1_found, global2_found, local1_found, local2_found, distinct1_found, distinct2_found, distinct3_found, y_found) %>% as_tibble()

componentShapes %>% select(-FMS,-index) %>% pivot_longer(-c(beta,pi)) %>% ggplot(aes(x=as.factor(beta),fill=as.factor(value))) + facet_grid(vars(pi), vars(name)) + geom_bar()
...
componentShapes %>% 
    ...
    select(beta,pi,Global, Local, Distinct) %>%
    pivot_longer(-c(beta,pi)) %>%
    ggplot(aes(x=as.factor(beta),fill=as.factor(value))) +
    facet_grid(vars(pi),vars(name)) +
    geom_bar(col="black") +
    xlab("Beta") + 
