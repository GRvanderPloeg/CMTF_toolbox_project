...
FMSdelta = matrix(NA, nrow=length(deltas), ncol=9)
...
for(i in 1:length(deltas)){
  ...
  FMSdelta[i,] = df %>% mutate(pi=pis) %>% filter(pi!=1) %>% select(C4,pi) %>% group_by(pi) %>% summarize(C4 = max(C4)) %>% select(C4) %>% pull()
  FMSdelta[i,] = FMSdelta[i,] - threshold
  
  a = df %>% mutate(pi=pis) %>% pivot_longer(-pi) %>% filter(name=="C4") %>% ggplot(aes(x=value,col=as.factor(pi))) + stat_ecdf() + geom_vline(xintercept=threshold, col="red", linetype="dashed", size=1.5) + xlab("FMS of the hidden local component") + ylab("ECDF(FMS)") + labs(col=expression(pi)) + ggtitle(delta)
  
  b= df %>% mutate(C1 = C1 > threshold, C2 = C2 > threshold, C3 = C3 > threshold, C4 = C4 > threshold, C5 = C5 > threshold, C6 = C6 > threshold, C7 = C7 > threshold, C8 = C8 > threshold) %>% select(-Overall) %>% mutate(pi=pis) %>% pivot_longer(-pi) %>% filter(name=="C4") %>% ggplot(aes(x=as.factor(pi),fill=value)) + geom_bar(col="black") + xlab(expression(pi)) + ylab("Number of models") + scale_fill_manual(name="FMS improvement", labels=c("No", "Yes"), values=hue_pal()(2)) + ggtitle(delta)
  
  print(ggarrange(a,b,nrow=2,ncol=1))
}
```
```{r FMS delta plot}
cbind(deltas, FMSdelta) %>% as_tibble() %>% pivot_longer(-deltas) %>% mutate(name = factor(name, levels=c(paste0("V", 2:10)))) %>% ggplot(aes(x=as.factor(name),y=as.factor(deltas),fill=value)) + geom_tile(col="black") + scale_fill_gradient2() + xlab(expression(pi)) + scale_x_discrete(labels=seq(0.1, 1, 0.1)[-10]) + ylab(expression(delta)) + labs(fill=expression(Delta*FMS))
```
