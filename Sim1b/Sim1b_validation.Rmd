...
library(vegan)
defaultModels = which(ACMTF_CV_params == 1)
ACMTF_models = ACMTF_CV_models[defaultModels]

ACMTF_f = do.call(rbind, lapply(ACMTF_models, FUN=function(x){x$f})) %>% as_tibble()
...
correctModels = ACMTF_f %>% mutate(index=1:nrow(ACMTF_f)) %>% filter(V1 < 1.035) %>% select(index) %>% pull()
# correctModels = ACMTF_varExps %>% filter(X1 > 86, X2 > 86, X3 > 86) %>% select(index) %>% pull()
...
ACMTF_CV_f %>% ggplot(aes(x=as.factor(noise),y=V1)) + geom_boxplot()
thresholds = ACMTF_CV_f %>% group_by(noise) %>% summarize(threshold = min(V1))
ACMTF_CV_f = ACMTF_CV_f %>% left_join(thresholds) %>% mutate(correct = V1 <= threshold)

ACMTF_CV_f %>% ggplot(aes(x=as.factor(noise),fill=as.factor(correct))) + geom_bar(col="black") + xlab("noise")
```
