...
ACMTF_CV_varExps = ACMTF_CV_varExps %>% mutate(noise = noise_levels)

ACMTF_CV_varExps %>% as_tibble() %>% pivot_longer(-noise) %>% ggplot(aes(x=as.factor(noise),y=value)) + facet_wrap(~name,nrow=3) + geom_boxplot() + xlab("Noise level") + ylab("Variance explained (%)")
```
...
ACMTF_CV_lambda_similarity = ACMTF_CV_lambda_similarity %>% as_tibble() %>% mutate(noise=noise_levels, index=1:length(noise))

ACMTF_CV_lambda_similarity %>% ggplot(aes(x=as.factor(noise),y=V1)) + geom_boxplot() + xlab("Noise level") + ylab("Lambda similarity") + scale_y_log10()
```
...
ACMTF_CV_FMS = ACMTF_CV_FMS %>% as_tibble() %>% mutate(X1 = FMS1 * FMS2 * FMS3, X2 = FMS1 * FMS4 * FMS5, X3 = FMS1 * FMS6 * FMS7, noise=noise_levels, index=1:length(noise_levels))

ACMTF_CV_FMS %>% select(X1,X2,X3,noise) %>% pivot_longer(-noise) %>% ggplot(aes(x=as.factor(noise),y=value)) + facet_wrap(~name, nrow=3) + geom_boxplot() + xlab("Noise level") + ylab("FMS")
```
...
ACMTF_CV_f %>% ggplot(aes(x=as.factor(noise),y=V1)) + geom_boxplot() + geom_hline(yintercept=0.25, col="red") + xlab("Noise level") + ylab("Loss value")
```
