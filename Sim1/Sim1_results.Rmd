...
```
```{r Age's idea}
# rhos = do.call(rbind, lapply(ACMTFR_models, FUN=function(x){t(x$rho)}))
```
...
ggplot(df, aes(x = factor(pi), y = Overall)) +
    geom_violin(scale = "width") +
    geom_jitter(width = 0.05, size = 0.5) +
    geom_point(data = max_pts, colour = "red", size = 2) + 
    ylim(0, 1) +
...
plot_df <- df %>% 
    mutate(pi   = pis,
           loss = sapply(ACMTFR_models, function(x) x$f)) %>% 
    group_by(pi) %>%                          # facet-wise minimum
    mutate(best = loss == min(loss, na.rm = TRUE)) %>% 
    ungroup()

ggplot(plot_df, aes(x = Overall, y = loss)) +
    facet_wrap(~pi) +
    ## first: all ordinary points (black)
    geom_point(data = subset(plot_df, !best), colour = "black") +
    ## second: the minima drawn on top (red, a bit larger)
    geom_point(data = subset(plot_df,  best), colour = "red",  size = 2) +
    xlab("Overall FMS") +
    ylab("Loss") +
    ylim(0, 1)

# Versus C4 specifically
plot_df <- df %>% 
    mutate(pi   = pis,
           loss = sapply(ACMTFR_models, function(x) x$f)) %>% 
    group_by(pi) %>%                          # facet-wise minimum
...
ggplot(plot_df, aes(x = C4, y = loss)) +
    facet_wrap(~pi) +
    ## first: all ordinary points (black)
    geom_point(data = subset(plot_df, !best), colour = "black") +
    ## second: the minima drawn on top (red, a bit larger)
    geom_point(data = subset(plot_df,  best), colour = "red",  size = 2) +
    xlab("FMS of hidden component") +
    ylab("Loss") +
    ylim(0, 1)

# Lowest loss model is the one with the highest FMS (except for pi <= 0.2)
# of the hidden component too!
```
