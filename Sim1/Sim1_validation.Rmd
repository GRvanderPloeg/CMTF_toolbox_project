...
ACMTF_pro = do.call(rbind, lapply(ACMTF_models, FUN=function(x){procrustes(x$Fac[[1]], inputLoadings[[1]])$ss})) %>% as_tibble()

# ACMTF_frob = do.call(rbind, lapply(ACMTF_models, FUN=function(x){norm(x$Fac[[1]] - inputLoadings[[1]], "F")})) %>% as_tibble()
# ACMTF_trace = do.call(rbind, lapply(ACMTF_models, FUN=function(x){sum(svd(x$Fac[[1]] - inputLoadings[[1]])$d)})) %>% as_tibble()
# ACMTF_temp = do.call(rbind, lapply(ACMTF_models, FUN=function(x){sum(svd(procrustes(x$Fac[[1]], inputLoadings[[1]])$Yrot - inputLoadings[[1]])$d)})) %>% as_tibble()
...
correctModels = which(ACMTF_pro <= 1e-2)
...
a=ACMTF_CV_varExps %>% pivot_longer(-c(beta,correct)) %>% ggplot(aes(x=as.factor(beta),y=value)) + facet_wrap(~name, nrow=3) + geom_boxplot(outliers=FALSE) + geom_point(aes(col=as.factor(correct)), position=position_jitter(width=0.05,height=0.0)) + xlab("beta") + ylab("Variation explained (%)")

b=ACMTF_CV_varExps %>% ggplot(aes(x=as.factor(beta),fill=as.factor(correct))) + geom_bar(col="black") + xlab("beta")
