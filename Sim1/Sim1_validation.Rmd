...
```{r alternative approach how often are components found}
...
componentShapes %>% select(-X1,-X2,-X3,-correct) %>% pivot_longer(-beta) %>% ggplot(aes(x=as.factor(beta),fill=as.factor(value))) + facet_wrap(~name,nrow=7) + geom_bar()
...
#model_dev = CMTFtoolbox::mize_runner(init, fg, abs_tol=abs_tol, rel_tol=rel_tol)
model_dev = readRDS("./model_dev.RDS")

# Not quite the same loss, but very similar solutions
f_per_block = apply(model_dev$all_iterations, 2, FUN=function(x){acmtf_fun(x,Z,alpha,beta,epsilon,manual=TRUE)[[1]]})
f_per_block = t(f_per_block) %>% as_tibble()

f_norm = apply(model_dev$all_iterations, 2, FUN=function(x){acmtf_fun(x,Z,alpha,beta,epsilon,manual=TRUE)[[2]]})
f_norm = colSums(f_norm)

f_lambda = apply(model_dev$all_iterations, 2, FUN=function(x){acmtf_fun(x,Z,alpha,beta,epsilon,manual=TRUE)[[3]]})
f_lambda = colSums(f_lambda)

df = cbind(f_per_block, f_norm, f_lambda)
colnames(df) = c("X1", "X2", "X3", "norm1", "lambda")
f_per_block %>% mutate(iteration = 1:nrow(.)) %>% pivot_longer(-iteration) %>% ggplot(aes(x=iteration,y=value,col=as.factor(name))) + geom_line()
