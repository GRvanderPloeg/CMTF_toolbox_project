...
ACMTF_FMS %>% as_tibble() %>% mutate(correct = ACMTF_FMS1>0.99 & ACMTF_FMS2>0.99 & ACMTF_FMS3>0.99 & ACMTF_FMS4>0.99 & ACMTF_FMS5>0.99 & ACMTF_FMS6>0.99 & ACMTF_FMS7>0.99) %>% mutate(index=1:nrow(.)) %>% pivot_longer(-c(correct,index)) %>% ggplot(aes(x=index,y=value,fill=as.factor(correct))) + facet_wrap(~name) + geom_bar(stat="identity")
