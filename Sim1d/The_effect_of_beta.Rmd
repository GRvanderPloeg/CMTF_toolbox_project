...
```{r load ACMTF models}
ACMTF_CV_models = readRDS("./Sim1d_ACMTF_CV_models_Y_inside.RDS")
ACMTF_CV_params = readRDS("./Sim1d_ACMTF_CV_params_Y_inside.RDS")
```
...
cbind(what, ACMTF_CV_params) %>%
  as_tibble() %>%
  mutate(size = paste0(numSubjects,"_",numFeatures,"_",numTimepoints)) %>%
  ggplot(aes(x=as.factor(size),fill=as.factor(what))) +
  facet_wrap(~beta) +
  geom_bar(col="black") +
  xlab(expression(beta)) +
  ylab("Number of models") +
  scale_fill_manual(name="Reason", labels=c("Absolute loss value", "Maximum number of function evaluations", "Relative change in loss value", "Step tol"), values=hue_pal()(4))
```
...
ACMTF_CV_varExps = cbind(ACMTF_CV_varExps, ACMTF_CV_params)

ACMTF_CV_varExps %>% as_tibble() %>% mutate(numParams = numSubjects * numFeatures * numTimepoints) %>% select(-numSubjects,-numFeatures,-numTimepoints) %>% pivot_longer(-c(beta,numParams)) %>% ggplot(aes(x=numParams,y=value,fill=as.factor(name))) + facet_wrap(~beta) + geom_point()
```
