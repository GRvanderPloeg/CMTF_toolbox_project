...
inputLoadings = readRDS("./Sim1e_input_loadings_Y_inside.RDS")
...
```{r load ACMTF models}
ACMTF_CV_models = readRDS("./Sim1e_ACMTF_CV_models_Y_inside.RDS")
ACMTF_CV_params = readRDS("./Sim1e_ACMTF_CV_params_Y_inside.RDS")
```
...
ACMTF_CV_varExps = ACMTF_CV_varExps %>% mutate(alpha = ACMTF_CV_params)

ACMTF_CV_varExps %>% 
    as_tibble() %>%
    pivot_longer(-alpha) %>%
    ggplot(aes(x=as.factor(alpha),y=value)) +
...
ACMTF_CV_lambda_similarity = ACMTF_CV_lambda_similarity %>%
  as_tibble() %>%
  mutate(alpha = ACMTF_CV_params, index = 1:length(alpha))

ACMTF_CV_lambda_similarity %>%
  ggplot(aes(x = as.factor(alpha), y = V1)) +
  geom_violin(scale="width") +
...
ACMTF_CV_FMS = ACMTF_CV_FMS %>% as_tibble() %>% mutate(X1 = FMS1 * FMS2 * FMS3, X2 = FMS1 * FMS4 * FMS5, X3 = FMS1 * FMS6 * FMS7, alpha=ACMTF_CV_params, index=1:length(alpha))

ACMTF_CV_FMS %>% 
  select(X1,X2,X3,alpha) %>%
  pivot_longer(-alpha) %>%
  ggplot(aes(x=as.factor(alpha),y=value)) +
  facet_wrap(~name, ncol=3) +
  geom_violin(scale="width") +
...
ACMTF_CV_FMS %>% select(X1,X2,X3,alpha) %>% pivot_longer(-alpha) %>% ggplot(aes(x=value)) + facet_grid(vars(alpha),vars(name)) + geom_histogram() + ylim(0,100) + xlab("Factor Match Score") + ylab("Number of models")
```
```{r plot loss}
...
ACMTF_CV_FMS %>% mutate(lambda_similarity = ACMTF_CV_lambda_similarity$V1) %>% select(X1, X2, X3, alpha, lambda_similarity) %>% pivot_longer(-c(alpha, lambda_similarity)) %>% ggplot(aes(x=lambda_similarity,y=value)) + facet_grid(vars(name), vars(alpha)) + geom_point() + scale_x_log10() + theme(legend.position="none") + xlab("Lambda similarity") + ylab("FMS")
```
...
ACMTF_CV_FMS %>%
  ...
  ggplot(aes(x=as.factor(alpha),fill=Correct)) +
  geom_bar(col="black") +
  xlab(expression(alpha)) +
...
componentShapes %>%
  ...
  select(alpha, Global, Local, Distinct) %>%
  pivot_longer(-alpha) %>%
  mutate(name = factor(name, levels=c("Global", "Local", "Distinct"))) %>%
  ggplot(aes(x=as.factor(alpha),fill=as.factor(value))) +
  facet_wrap(~name, nrow=3) +
  geom_bar(col="black") +
  labs(x=expression(alpha),y="Number of models",fill="Number of components lost") +
