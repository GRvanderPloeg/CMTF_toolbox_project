...
models = foreach::foreach(i=1:length(betas)) %dopar% {
  model=CMTFtoolbox::acmtf_opt(Z,numComponents=8,initialization="random",beta=rep(betas[i],3),abs_tol=1e-10,rel_tol=1e-10,nstart=1)
}
parallel::stopCluster(cl)

# Save model and parameters
saveRDS(models, "Sim4_ACMTF_CV_models_Y_outside_inside_8comp.RDS")
saveRDS(betas, "Sim4_ACMTF_CV_params_Y_outside_inside_8comp.RDS")
