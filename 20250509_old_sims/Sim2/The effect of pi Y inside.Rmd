...
ACMTFR_CV_models_addon = readRDS("./Sim2_ACMTFR_CV_Y_inside_addon.RDS")
ACMTFR_CV_params_addon = readRDS("./Sim2_ACMTFR_CV_params_Y_inside_addon.RDS")

ACMTFR_CV_models_united = c(ACMTFR_CV_models, ACMTFR_CV_models_addon)
ACMTFR_CV_params_united = rbind(ACMTFR_CV_params, ACMTFR_CV_params_addon)
```
...
mask2 = ACMTFR_CV_params_addon[,1] == 1e-3
models2 = ACMTFR_CV_models_addon[mask2]
pi_values2 = ACMTFR_CV_params_addon[mask2,2]

mask3 = ACMTFR_CV_params_united[,1] == 1e-3
models_united = ACMTFR_CV_models_united[mask3]
pi_values_united = ACMTFR_CV_params_united[mask3,2]
```

```{r plot varExps}
ACMTF_CV_varExpsX = do.call(rbind, lapply(models, FUN=function(x){x$varExp*100})) %>% as_tibble()
...
ACMTF_CV_varExps1 = ACMTF_CV_varExpsX %>% mutate(Y=ACMTF_CV_varExpsY$V1, pi = pi_values)

ACMTF_CV_varExpsX2 = do.call(rbind, lapply(models2, FUN=function(x){x$varExp})) %>% as_tibble()
ACMTF_CV_varExpsY2 = do.call(rbind, lapply(models2, FUN=function(x){(x$varExpY)})) %>% as_tibble()
colnames(ACMTF_CV_varExpsX2) = c("X1", "X2", "X3")
ACMTF_CV_varExpsX2 = ACMTF_CV_varExpsX2 %>% mutate(pi = pi_values2)
ACMTF_CV_varExps2 = ACMTF_CV_varExpsX2 %>% mutate(Y=ACMTF_CV_varExpsY2$V1, pi = pi_values2)

ACMTF_CV_varExps = rbind(ACMTF_CV_varExps1, ACMTF_CV_varExps2) %>% as_tibble()
...
ACMTF_CV_lambda_similarity = do.call(rbind, lapply(models_united, FUN=function(x){lambda_similarity(x$Fac[[8]])}))
ACMTF_CV_lambda_similarity = ACMTF_CV_lambda_similarity %>% as_tibble() %>% mutate(pi=pi_values_united, index=1:length(pi))
...
a=models_united[[6]]$Fac[[8]] %>% as_tibble() %>% mutate(index=1:nrow(.)) %>% pivot_longer(-index) %>% mutate(index = factor(index, levels=c(3,2,1))) %>% ggplot(aes(y=index,x=as.factor(name),fill=value)) + geom_tile(col="black") + ylab("X block") + scale_x_discrete(name="Component number", labels=1:7) + scale_fill_gradient2(low="red",high="green",mid="grey",midpoint=0) + geom_text(aes(label=round(value,2))) + scale_y_discrete(name="X block", labels=c("X3", "X2", "X1")) + ggtitle("Pi=1") + theme(legend.position="none")

b=models_united[[17]]$Fac[[8]] %>% as_tibble() %>% mutate(index=1:nrow(.)) %>% pivot_longer(-index) %>% mutate(index = factor(index, levels=c(3,2,1))) %>% ggplot(aes(y=index,x=as.factor(name),fill=value)) + geom_tile(col="black") + ylab("X block") + scale_x_discrete(name="Component number", labels=1:7) + scale_fill_gradient2(low="red",high="green",mid="grey",midpoint=0) + geom_text(aes(label=round(value,2))) + scale_y_discrete(name="X block", labels=c("X3", "X2", "X1")) + ggtitle("Pi=0.8") + theme(legend.position="none")

c=models_united[[9]]$Fac[[8]] %>% as_tibble() %>% mutate(index=1:nrow(.)) %>% pivot_longer(-index) %>% mutate(index = factor(index, levels=c(3,2,1))) %>% ggplot(aes(y=index,x=as.factor(name),fill=value)) + geom_tile(col="black") + ylab("X block") + scale_x_discrete(name="Component number", labels=1:7) + scale_fill_gradient2(low="red",high="green",mid="grey",midpoint=0) + geom_text(aes(label=round(value,2))) + scale_y_discrete(name="X block", labels=c("X3", "X2", "X1")) + ggtitle("Pi=0.4") + theme(legend.position="none")

d=models_united[[1]]$Fac[[8]] %>% as_tibble() %>% mutate(index=1:nrow(.)) %>% pivot_longer(-index) %>% mutate(index = factor(index, levels=c(3,2,1))) %>% ggplot(aes(y=index,x=as.factor(name),fill=value)) + geom_tile(col="black") + ylab("X block") + scale_x_discrete(name="Component number", labels=1:7) + scale_fill_gradient2(low="red",high="green",mid="grey",midpoint=0) + geom_text(aes(label=round(value,2))) + scale_y_discrete(name="X block", labels=c("X3", "X2", "X1")) + ggtitle("Pi=0") + theme(legend.position="none")


ggarrange(a,b,c,d, nrow=1, ncol=4)
```
```{r plot fms}
ACMTF_CV_FMS1 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[1]], inputLoadingsFac[[1]], numComponents=7)})) %>% as_tibble()
ACMTF_CV_FMS2 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[2]], inputLoadingsFac[[2]], numComponents=5)})) %>% as_tibble()
ACMTF_CV_FMS3 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[3]], inputLoadingsFac[[3]], numComponents=5)})) %>% as_tibble()
ACMTF_CV_FMS4 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[4]], inputLoadingsFac[[4]], numComponents=5)})) %>% as_tibble()
ACMTF_CV_FMS5 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[5]], inputLoadingsFac[[5]], numComponents=5)})) %>% as_tibble()
ACMTF_CV_FMS6 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[6]], inputLoadingsFac[[6]], numComponents=3)})) %>% as_tibble()
ACMTF_CV_FMS7 = do.call(rbind, lapply(models_united, FUN=function(x){calculateFMS(x$Fac[[7]], inputLoadingsFac[[7]], numComponents=3)})) %>% as_tibble()
...
ACMTF_CV_FMS = ACMTF_CV_FMS %>% as_tibble() %>% mutate(X1 = FMS1 * FMS2 * FMS3, X2 = FMS1 * FMS4 * FMS5, X3 = FMS1 * FMS6 * FMS7, pi=pi_values_united, index=1:length(pi))
...
```{r plot loss}
ACMTF_CV_f = do.call(rbind, lapply(models_united, FUN=function(x){x$f}))
ACMTF_CV_f = ACMTF_CV_f %>% as_tibble() %>% mutate(pi=pi_values_united, index=1:length(pi))
...
global1 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,1])))}))
global2 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,2])))}))
local1 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,3])))}))
local2 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,4])))}))
distinct1 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,5])))}))
distinct2 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,6])))}))
distinct3 = unlist(lapply(models_united, FUN=function(x){max(abs(cor(x$Fac[[1]], inputLoadings[[1]][,7])))}))
y_found = unlist(lapply(models_united, FUN=function(x){abs(cor(x$Yhat, Yreal))}))
