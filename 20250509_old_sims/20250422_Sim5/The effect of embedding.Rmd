...
```{r check CV outcome}
cv_095 = readRDS("./Sim5_ACMTFR_model_selection_result0.95.RDS")
```
...
df %>% as_tibble() %>% mutate(pi=pi_values) %>% pivot_longer(-pi) %>% ggplot(aes(x=as.factor(name),y=value)) + facet_wrap(~pi) + geom_boxplot()

cbind(unlist(lapply(ACMTFR_CV_models, FUN=function(x){max(abs(x$rho))})), pi_values) %>% as_tibble() %>% ggplot(aes(x=as.factor(pi_values),y=V1)) + geom_boxplot()
```
