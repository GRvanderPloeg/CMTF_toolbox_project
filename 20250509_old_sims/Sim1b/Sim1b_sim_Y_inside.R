...
for(i in 1:length(noises)){
  ...
  noise1_final = fnorm(X1_raw) / (1/noiseOnX) * (noise1 / fnorm(noise1))
  noise2_final = fnorm(X2_raw) / (1/noiseOnX) * (noise2 / fnorm(noise2))
  noise3_final = fnorm(X3_raw) / (1/noiseOnX) * (noise3 / fnorm(noise3))

  # Add noise to X blocks
  X1_noise = X1_raw + noise1_final
  X2_noise = X2_raw + noise2_final
  X3_noise = X3_raw + noise3_final
  ...
  # Save sim
  saveRDS(X1_final, paste0("./Sim1b/Sim1b_X1_Y_inside_", noiseOnX, ".RDS"))
  saveRDS(X2_final, paste0("./Sim1b/Sim1b_X2_Y_inside_", noiseOnX, ".RDS"))
  saveRDS(X3_final, paste0("./Sim1b/Sim1b_X3_Y_inside_", noiseOnX, ".RDS"))
}

saveRDS(Y_final, "./Sim1b/Sim1b_Y_Y_inside.RDS")
