...
```{r check CV}
WHZ1 = readRDS("./CV_small_WHZ_0.25.RDS")
WHZ2 = readRDS("./CV_small_WHZ_0.5.RDS")
WHZ3 = readRDS("./CV_small_WHZ_0.75.RDS")
WHZ4 = readRDS("./CV_small_WHZ_0.9.RDS")
WHZ5 = readRDS("./CV_small_WHZ_0.95.RDS")
WHZ6 = readRDS("./CV_small_WHZ_1.RDS")
...
filteredMetadata = Jakobsen2025$homogenizedSubjectMetadata[mask,]
```
...
plotModel = function(model, comp=1){
  a = filteredMetadata %>% mutate(loading = model$Fac[[1]][,comp]) %>% arrange(BMI.group) %>% mutate(index=1:nrow(.)) %>% ggplot(aes(x=index,y=loading,fill=as.factor(BMI.group))) + geom_bar(stat="identity") + theme(legend.position = "na")
...
plotModel(model_whz25)
plotModel(model_whz50)
```
