...
pis = seq(from=0.8, to=0.95, length.out=10)
models = list()
for(i in 1:length(pis)){
  pi = pis[i]
  print(pi)
  models[[i]] = CMTFtoolbox::acmtfr_opt(Z, Ynorm, 2, pi=pi, nstart=100, numCores=parallel::detectCores(), method="L-BFGS", allOutput=FALSE)
}

saveRDS(models, "CV_BMI_pi_models3.RDS")
