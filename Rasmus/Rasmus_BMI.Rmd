...
pis = seq(from=0.9, to=1, length.out=11)
varExpsY = rep(NA, length(pis))
for(i in 1:length(pis)){
  pi = pis[i]
  print(pi)
  varExpsY[i] = CMTFtoolbox::acmtfr_opt(Z, Ycnt, 1, pi=pi, nstart=12, numCores=12, method="L-BFGS")$varExpY
}

cbind(pis, varExpsY) %>% ggplot(aes(x=pis,y=varExpsY)) +
  geom_point() +
  geom_line() +
  xlab("Pi") +
