...
# varExpsX = do.call(rbind, lapply(CV_pi_models, FUN=function(x){x$varExp}))
varExpsY = do.call(rbind, lapply(CV_pi_models, FUN=function(x){x$varExpY}))

cbind(pis, varExpsY) %>% 
  as_tibble() %>%
  ggplot(aes(x=as.factor(pis),y=V2,group=1)) +
  geom_path() +
  geom_point() +
  xlab(expression(pi)) +
