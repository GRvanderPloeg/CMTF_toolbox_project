...
df = c(NPLStoolbox::Jakobsen2025$faeces$mode1$subject, NPLStoolbox::Jakobsen2025$milkMicrobiome$mode1$subject, NPLStoolbox::Jakobsen2025$milkMetabolomics$mode1$subject) %>% table() %>% as_tibble() %>% filter(n==3)
homogenizedSamples = df[df$n == 3,1] %>% pull()
...
methods = c("pi=0.25 (1)","pi=0.50 (1)","pi=0.75 (1)","pi=0.90 (1)","pi=0.90 (2)","pi=0.95 (1)","pi=0.95 (2)","pi=0.95 (3)","pi=1 (1)", "pi=1 (2)")
...
NPLS_faecal = Jakobsen2025$subjectMeta_BMI %>% left_join(NPLS_faeces_mode1 %>% mutate(subject = as.character(subject))) %>% select(loading) %>% pull()
NPLS_milk = Jakobsen2025$subjectMeta_BMI %>% left_join(NPLS_milk_mode1 %>% mutate(subject = as.character(subject))) %>% select(loading) %>% pull()
NPLS_milkMetab = Jakobsen2025$subjectMeta_BMI %>% left_join(NPLS_milkMetab_mode1 %>% mutate(subject = as.character(subject))) %>% select(loading) %>% pull()

df = cbind(NPLS_faecal, NPLS_milk, NPLS_milkMetab, model_bmi25$Fac[[1]], model_bmi50$Fac[[1]], model_bmi75$Fac[[1]], model_bmi90$Fac[[1]], model_bmi95$Fac[[1]], model_bmiX_2$Fac[[1]])
...
methods = c("NPLS Faecal (1)", "NPLS Milk (1)", "NPLS MilkMetab (1)", "pi=0.25 (1)","pi=0.50 (1)","pi=0.75 (1)","pi=0.90 (1)","pi=0.90 (2)","pi=0.95 (1)","pi=0.95 (2)","pi=0.95 (3)", "pi=1 (1)", "pi=1 (2)")
...
df = cbind(NPLS_faeces_mode1$loading, model_bmi25$Fac[[4]], model_bmi50$Fac[[4]], model_bmi75$Fac[[4]], model_bmi90$Fac[[4]], model_bmi95$Fac[[4]], model_bmiX_2$Fac[[4]])
...
methods = c("NPLS (1)","pi=0.25 (1)","pi=0.50 (1)","pi=0.75 (1)","pi=0.90 (1)","pi=0.90 (2)","pi=0.95 (1)","pi=0.95 (2)","pi=0.95 (3)", "pi=1 (1)", "pi=1 (2)")

congruence_matrix %>%
  as_tibble() %>%
  mutate(index=1:11) %>%
  pivot_longer(-index) %>%
  mutate(name = factor(name, levels=paste0("V",1:11))) %>%
  ggplot(aes(x=as.factor(index),y=name,fill=value)) +
  geom_tile(col="black") +
  # geom_text(aes(label=round(value,2))) +
  scale_y_discrete(name="", labels=methods) +
